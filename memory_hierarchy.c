...
static int sweep_iters(size_t bytes) {
  ...
  if (bytes <= 512UL * 1024)
    return 2000000;
  if (bytes <= 4UL * 1024 * 1024)
    return 500000;
  if (bytes <= 64UL * 1024 * 1024)
    return 200000;
  return 100000;
}
...
static void derive_bench_sizes(const size_t *b, int nb, size_t *l1, size_t *l2,
  ...
  *ram = (nb >= 1) ? b[nb - 1] * 4 : 128UL * 1024 * 1024;
  ...
  if (*ram > 256UL * 1024 * 1024)
    *ram = 256UL * 1024 * 1024;
  if (*ram < 64UL * 1024 * 1024)
    *ram = 64UL * 1024 * 1024;
}
...
int main(void) {
...

  {
    ...
    printf("  %-28s  ", label);
    fflush(stdout);
    l1_ns = bench_cache(sz_l1, 5000000, 50);
    ...
    printf("  %-28s  ", label);
    fflush(stdout);
    l2_ns = bench_cache(sz_l2, 2000000, 50);
  ...
  if (reg_ns > 0.0) {
    ...
    if (l1_ns > 0.0)
      printf("    L1 cache:        %7.0fx\n", l1_ns / reg_ns);
    if (l2_ns > 0.0)
      printf("    L2 cache:        %7.0fx\n", l2_ns / reg_ns);
    if (llc_ns > 0.0)
      printf("    LLC:             %7.0fx\n", llc_ns / reg_ns);
    if (ram_ns > 0.0)
      printf("    Main memory:     %7.0fx\n", ram_ns / reg_ns);
    if (file_us > 0.0)
